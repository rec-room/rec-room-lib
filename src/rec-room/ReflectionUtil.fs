...
module ReflectionUtil =
    ...
    // Modified to cache results by jgb
    ...
    type DuMappings = {
        CaseToName : System.Collections.Generic.IDictionary<obj, string>
        NameToCase : System.Collections.Generic.IDictionary<string, obj>
    }
    let duMap = new System.Collections.Concurrent.ConcurrentDictionary<Type, DuMappings>()
    let genMappings (t:Type) =
        let pairs =
            FSharpType.GetUnionCases(t)
            |> Array.toList
            |> List.map (fun case ->
                    let n = case.Name
                    (t.GetMethod("get_"+n).Invoke(null, [||]), n))

        {
            CaseToName = dict (pairs |> List.map (fun (case, name) -> (case, name)) |> List.toSeq)
            NameToCase = dict (pairs |> List.map (fun (case, name) -> (name, case)) |> List.toSeq)
        }
    let duMappings t =
        duMap.GetOrAdd(t, genMappings)

    let toString2 (x:'a) =
        let duType = typeof<'a>
        let mappings = duMappings duType
        mappings.CaseToName.Item x

    let tryGetCase t str =
        let mappings = duMappings t
        try Some (mappings.NameToCase.Item(str))
        with
        | :? System.Collections.Generic.KeyNotFoundException as ex -> None

    let fromString2<'a> str =
        let duType = typeof<'a>
        tryGetCase duType str |> Option.map (fun x -> x :?> 'a)
    ...
    let rec getFields t prefix : string[] =
            if FSharpType.IsRecord t then
                    FSharpType.GetRecordFields t
                    |> Array.collect (fun pi ->
                    if FSharpType.IsRecord (pi.PropertyType) then
                            getFields pi.PropertyType (if prefix = "" then pi.Name else prefix + "." + pi.Name  )
                    else if isOption (pi) then
                            let propertyType = pi.PropertyType.GenericTypeArguments.[0]
                            getFields propertyType (if prefix = "" then pi.Name else prefix + "." + pi.Name  )
                    else if FSharpType.IsUnion (pi.PropertyType) then
                            getFields pi.PropertyType (if prefix = "" then pi.Name else prefix + "." + pi.Name  )
                    else
                            let n = if prefix = "" then pi.Name else prefix + "." + pi.Name
                            [| n |]
                    )
